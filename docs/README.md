...
computes optical-flow motion, applies an optional freehand exclusion mask,
combines intensity/motion/cone evidence into a binary spray mask, extracts
penetration and cone-angle metrics from the mask boundary, displays diagnostic
windows, saves an overlay video, asks for one manual validation mask, plots
metrics, and writes a per-frame CSV.
